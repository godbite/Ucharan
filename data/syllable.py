...
CONSONANT_LETTERS = tuple(range(0x0915, 0x0939 + 1)) + \
    tuple(range(0x0958, 0x095F + 1))
...
SYLLABLE_RE = rf"(?:({VOWEL_LETTERS_RE})|({CONSONANT_LETTERS_RE})({
    NUKTA_SIGN_RE}?)({VOWEL_SIGNS_RE}?))({DIACRITICAL_MARK_RE}?)|({DIGIT_RE})"
...
def load_text(start: int = 0, count: int = 10) -> str:
    return "\n\n".join(
        file_path.read_text() for file_path in speech_file_paths[start: start + count]
    )


def normalize_text(text: str) -> str:
    # Change consecutive whitespaces into a SPACE.
...
def show_devanagari_unicode_block():
    for c in range(0x0900, 0x097F + 1):
        print(f"U+{c:04X}", unicodedata.category(chr(c)),
